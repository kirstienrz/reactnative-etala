...
const SuperAdminSidebar = ({ isOpen, setIsOpen }) => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed inset-0 bg-black/50 z-[999] lg:hidden backdrop-blur-sm transition-opacity"
          onClick={() => setIsOpen(false)}
        />
      )}

      <div className={`
        fixed inset-y-0 left-0 z-[1000] lg:relative
        flex flex-col bg-gray-900 text-white h-screen transition-all duration-300
        ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
        ${isCollapsed ? 'lg:w-20' : 'w-64'}
      `}>
        {/* Header */}
        <div className={`px-6 pb-6 sidebar-header-pt border-b border-gray-700 flex items-center ${isCollapsed ? 'lg:justify-center' : 'justify-between'}`}>
          {(!isCollapsed || isOpen) && (
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="hidden lg:block p-2 rounded-lg hover:bg-gray-800 hover:text-white transition-colors text-gray-400"
            ...
            <button
              onClick={() => setIsOpen(false)}
              className="lg:hidden p-2 rounded-lg hover:bg-gray-800 hover:text-white transition-colors text-gray-400"
            >
              <X size={20} />
            </button>
          </div>
        ...
        >
          ...
          <nav>
            ...
            {/* CONTENT MANAGEMENT */}
            <SidebarSection title="Content Management" isCollapsed={isCollapsed && !isOpen}>
              <SidebarItem icon={<Image size={18} />} label="Highlights" to="/superadmin/carousel" isCollapsed={isCollapsed && !isOpen} onClick={() => setIsOpen(false)} />
              ...
              <SidebarItem icon={<Newspaper size={18} />} label="News & Announcements" to="/superadmin/news" isCollapsed={isCollapsed && !isOpen} onClick={() => setIsOpen(false)} />
              <SidebarItem icon={<Briefcase size={18} />} label="Projects" to="/superadmin/projects" isCollapsed={isCollapsed && !isOpen} onClick={() => setIsOpen(false)} />
              <SidebarItem icon={<DollarSign size={18} />} label="Budget & Programs" to="/superadmin/budget" isCollapsed={isCollapsed && !isOpen} onClick={() => setIsOpen(false)} />
              <SidebarItem icon={<Award size={18} />} label="Accomplishments" to="/superadmin/accomplishments" isCollapsed={isCollapsed && !isOpen} onClick={() => setIsOpen(false)} />
              <SidebarItem icon={<FileText size={18} />} label="Policies & Issuances" to="/superadmin/policies" isCollapsed={isCollapsed && !isOpen} onClick={() => setIsOpen(false)} />
              <SidebarItem icon={<FileSpreadsheet size={18} />} label="Organizational Chart" to="/superadmin/organizational" isCollapsed={isCollapsed && !isOpen} onClick={() => setIsOpen(false)} />
            </SidebarSection>

            {/* ADMIN TOOLS */}
            <SidebarSection title="Admin Tools" isCollapsed={isCollapsed && !isOpen}>
              <SidebarItem icon={<CalendarDays size={18} />} label="Calendar" to="/superadmin/events" isCollapsed={isCollapsed && !isOpen} onClick={() => setIsOpen(false)} />
              <SidebarItem icon={<Lightbulb size={18} />} label="Suggestion Box" to="/superadmin/suggestions" isCollapsed={isCollapsed && !isOpen} onClick={() => setIsOpen(false)} />
              <SidebarItem icon={<FileSignature size={18} />} label="Documentation Hub" to="/superadmin/documentation" isCollapsed={isCollapsed && !isOpen} onClick={() => setIsOpen(false)} />
              <SidebarItem icon={<DollarSign size={18} />} label="Finance" to="/superadmin/finance" isCollapsed={isCollapsed && !isOpen} onClick={() => setIsOpen(false)} />
            </SidebarSection>
          </nav>
        </div>

        {/* LOGOUT */}
        <div className="border-t border-gray-700 px-4 pt-4 pb-[max(env(safe-area-inset-bottom),1rem)]">
          <button
