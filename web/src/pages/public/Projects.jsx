    ...
    <main className="bg-white min-h-screen">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {allCategories.map((category) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium flex items-center gap-1 transition-colors ${selectedCategory === category.value
                    ? 'bg-blue-600 text-white'
                                        ...
                                        <div className="flex items-center gap-3 flex-1">
                                          ...
                                          <div className="flex-1">
                                            <div className="flex items-center gap-2">
                                              ...
                                              <span className={`text-xs px-2 py-0.5 rounded-full ${event.extendedProps?.type === 'holiday' ? 'bg-red-100 text-red-700' :
      ...
      {showFilePreview && selectedFile && (
        <div className={`fixed inset-0 bg-black ${isFullscreen ? 'bg-black' : 'bg-opacity-90'} flex items-center justify-center z-[9999] p-4 transition-all`}> {/* Modal overlay covers entire viewport, sits above header */}
          ...
          <div className="absolute top-4 sm:top-8 left-0 w-full flex flex-col sm:flex-row items-center sm:justify-between z-30 px-4 sm:px-8 gap-3 sm:gap-4 pointer-events-none">
            {/* Info bar for ALL file types */}
            <div className="flex-1 w-full sm:w-auto flex items-center bg-black bg-opacity-75 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-xl max-w-2xl pointer-events-auto shadow-2xl backdrop-blur-md border border-white/10">
              <div className="flex-1 min-w-0">
                <p className="text-xs sm:text-sm font-medium truncate">
                  {selectedFile.originalname || selectedFile.filename || 'File'}
                </p>
                <div className="flex items-center gap-3 mt-0.5 sm:mt-1 text-[10px] sm:text-xs text-gray-300">
                  {selectedFile.size && selectedFile.size > 0 && (
                    <span>{formatFileSize(selectedFile.size)}</span>
                  )}
                  {selectedFile.mimetype && (
                    <span className="uppercase bg-gray-700/50 px-1.5 py-0.5 rounded">
            ...
            {/* Action buttons */}
            <div className="flex gap-2 pointer-events-auto self-end sm:self-center">
              <button
                onClick={toggleFullscreen}
                className="text-white hover:text-gray-300 bg-black/50 hover:bg-black/70 backdrop-blur-md rounded-full p-2 transition-all border border-white/10"
              ...
              <button
                ...
                className="text-white hover:text-red-400 bg-black/50 hover:bg-red-500/20 backdrop-blur-md rounded-full p-2 transition-all border border-white/10"
                title="Close"
              >
          ...
          <div className={`relative ${isFullscreen ? 'w-screen h-screen' : 'max-w-6xl w-full max-h-[90vh]'}`}>
            ...
            {previewFiles.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-2 sm:left-4 top-1/2 transform -translate-y-1/2 text-white hover:text-gray-300 bg-black/40 hover:bg-black/60 shadow-lg backdrop-blur-sm rounded-full p-2 sm:p-3 z-20 group"
                >
                  <ChevronLeft className="w-5 h-5 sm:w-6 sm:h-6 group-hover:-translate-x-0.5 transition-transform" />
                </button>
                <button
                  onClick={handleNextPreview}
                  className="absolute right-2 sm:right-4 top-1/2 transform -translate-y-1/2 text-white hover:text-gray-300 bg-black/40 hover:bg-black/60 shadow-lg backdrop-blur-sm rounded-full p-2 sm:p-3 z-20 group"
                >
            ...
            {/* File counter */}
            {previewFiles.length > 1 && (
              <div className="absolute top-4 left-4 text-[10px] sm:text-xs text-white bg-black/40 backdrop-blur-md px-3 py-1 rounded-full z-10 font-bold border border-white/10 uppercase tracking-widest hidden sm:block">
